    ...
    config(Joi) {
      ...
      if (process.env.VCAP_SERVICES) {
        ...
        for (var svcName in services) {
          if (svcName.match(/^seachly/) || svcName.match(/elasticsearch$/)) {
            var elasticsearchCreds = services[svcName][0]['credentials'];
            elasticsearchUri = elasticsearchCreds.uri;
          }
        }
      }
      ...
      return object({
        ...
        url: Joi.string().uri({ scheme: ['http', 'https'] })['default'](elasticsearchUri || 'http://localhost:9200'),
